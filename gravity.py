...
class GravityFrame(tk.Frame):
    def __init__(self, master=None, precision=None, **kwargs):
        ...
        self.label_mass = tk.Label(self, text="Mass (kg):")
        self.label_mass.grid(row=0, column=0)
        self.entry_mass = tk.Entry(self)
        ...
        self.label_g_result.grid(row=4, columnspan=2)
    def calculate_gravity(self,precision):
        try:
            ...
            self.label_g_result.config(text=f'Acceleration due to gravity = {result:.{precision.get()}f} m/s^2')
            self.label_g_result.grid(row=4, columnspan=2)
        except ValueError:
